...
import (
	...
	"github.com/nathabuddhi/ay-com/backend/util-media/server"
	"go.uber.org/zap"
...
func main() {
	...
	pb.RegisterMediaServiceServer(grpcServer, server.NewMediaServer())

	zap.L().Info("Redis Service gRPC server started successfully.")

	zap.L().Info("Redis Service Running. Listening on port 5013.")
