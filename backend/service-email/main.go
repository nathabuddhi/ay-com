...
import (
	...
	"github.com/nathabuddhi/ay-com/backend/service-email/server"
	"go.uber.org/zap"
...
func main() {
	...
	pb.RegisterEmailServiceServer(grpcServer, server.NewEmailServer())

	zap.L().Info("Redis Service gRPC server started successfully.")

	zap.L().Info("Redis Service Running. Listening on port 5011.")
